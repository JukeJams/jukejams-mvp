...
router.post('/login', async (req, res) => {
  ...
  if (user === null) {
    await User.create(req.body).then((dbResponse) => {
    ...
    if (playlist) {
      ...
      if (playlistSongs) {
        ...
        const songs = playlistSongs.map((song) =>
          Song.findByPk(song.songId, { raw: true })
        );

        await Promise.all(songs).then((mapped) =>
          res.send({ user, songs: mapped })
        );
        return;
...
router.put('/vote', async (req, res) => {
  ...
  if (reset === true) {
    ...
    const partySongUser = await PartySongUser.findOne({
      where: { partyId: party.id, songId: song.id, userId },
    });
    if (partySongUser === null) {
      PartySongUser.create({ partyId: party.id, songId: song.id, userId });
      playlistSong.update(voteObj).then(() => {
...
router.post('/host', async (req, res) => {
  ...
  if (host === false) {
    ...
    if (party === null) {
      ...
      Party.create({ hostId: id, accessCode }).then(({ dataValues }) => {
...
router.post('/playlist/:user', async (req, res) => {
  ...
  let playlist_song = await PlaylistSong.findOne({
